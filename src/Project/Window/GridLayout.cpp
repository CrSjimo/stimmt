...
namespace stimmt {
    QJSValue GridLayout::createScriptObject() {
        return ObjectWrapper::wrap(this, jsGlobal->engine(),
                                   {"addElement", "addElement", "rowMinimumHeight", "columnMinimumWidth",
                                    ...
                                    "setRowStretch", "setColumnStretch", "horizontalSpacing", "verticalSpacing",
                                    "rowCount", "columnCount", "count", "originCorner"});
    }
    ...
    void GridLayout::addElement(const QJSValue &jsWidget, int row, int column, int rowSpan, int columnSpan,
                                int alignment) {
        auto widget = ObjectWrapper::getOfWrappedObject<QWidget>(jsWidget);
        if (widget) {
