...
namespace stimmt {
    ...
    GlobalObject::GlobalObject(QObject * parent) : QObject(parent), m_engine(new QJSEngine(this)) {
    ...
    QString GlobalObject::stackTrace(int depth) {
        return m_engine->evaluate("throw new Error()")
            .property("stack")
            .toString()
            .split("\n")
            .mid(1 + depth)
    ...
    void GlobalObject::interruptExecution() {
        ...
        QTimer::singleShot(0, [=] { m_engine->setInterrupted(false); });
    }
