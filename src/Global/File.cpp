...
namespace stimmt {
    ...
    QJSValue File::lastModifiedDate() const {
        ...
        return jsGlobal->engine()->globalObject().property("Date").callAsConstructor(
            {date.toString(Qt::ISODateWithMs)});
    }
    ...
    int File::write(const QJSValue &data) {
        ...
        if (data.isString()) {
            ...
            return write(
                jsGlobal->engine()->globalObject().property("Uint8Array").callAsConstructor({data}).property("buffer"));
